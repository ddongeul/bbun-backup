    ...
    public void onHttpResponse(String result) {
        ...
        if (result != null) {
            try {
                ...
                else if(loginResult.equals("notAuth")){ // 학생 인증이 안된경우
                    Toast.makeText(getApplicationContext(), "인증을 해주세요.", Toast.LENGTH_SHORT).show();
                }

             else{ //아이디와 비밀번호를 모두 잘 입력한 경우

                    String account = responseBody.getString("account");
                    LoginUser.initInstance(account);//사용자 정보 초기화
