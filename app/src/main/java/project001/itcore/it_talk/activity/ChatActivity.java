...
public class ChatActivity extends AppCompatActivity {
    ...
    protected void onResume(){
        ...
        Log.d(TAG,"[intent] nickName : "+ intent.getExtras().getString("nickName"));  //추후 학번이나 아이디로수정
    ...
    private void emitSocket(String message){
        ...
        try {
            ...
            if(chat_type == 0){
                ...
                sendMessage.put("name", account); // 서버에 로그인 한 아이디 값 전달
    ...
    private void start_Random(){
        ...
        // 서버에 account 명령을 전송  아이디 값을 서버로 전송
        JSONObject accountMessage = new JSONObject();
        try {
            accountMessage.put("message",account);

        } catch (JSONException e) {
            e.printStackTrace();
        }
        mSocket.emit("account",accountMessage);
        Log.d(TAG,"[account] " + accountMessage);
...
//          joinMessage.put("name", user.getNickName() );
            ...
            joinMessage.put("serial", loginUser.getAccount());
        } catch (JSONException e) {
            e.printStackTrace();
        }
